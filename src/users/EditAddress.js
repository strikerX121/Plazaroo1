...
export default class Addadress extends Component{
  ...
  getaddressdata=()=>{
       ...
       {
         this.setState({other:false,home:true,work:false,address_type:0,placename:data.place_name,address:data.address})
       }
       else if(data.address_type==1){
        this.setState({other:false,home:false,work:true,address_type:1,placename:data.place_name,address:data.address})
       }
       else if(data.address_type==2){
        this.setState({other:true,home:false,work:false,address_type:2,placename:data.place_name,address:data.address})
       }
   ...
   addaddressbtn = async() => {
      let userdata=await localStorage.getItemObject('user_arr')
        //-------------------- input validations -----------------
let data2=this.state.addressdata
        let add={'address':data2.address,'latitude':data2.latitude,'longitude':data2.longitude}
     let user_id=userdata.user_id
      ...
      data.append('address_id',this.state.addressdata.address_id)
      ...
      if(selleraddress!='NA')
      {
        ...
        data.append('latitude',add.latitude)
        data.append('longitude',add.longitude)
      }
     ...
     fetch(url,{
        ...
        //msgProvider.alert(msgTitle.response[config.language], JSON.stringify(obj), false);
           if(obj.success == 'true'){
            selleraddress='NA'
            this.props.navigation.navigate('Youraddress');
   ...
   <View style={styles.inputcontainer}>
         ...
         <View style={[styles.inputcontainer,{ borderColor:Colors.inputborder, borderWidth:1.5,paddingVertical:7}]}>
             <View style={{flexDirection:'row',paddingLeft:10,paddingVertical:7}}>
              ...
              { selleraddress!='NA' && <Text style={{fontSize:13,paddingLeft:10,fontFamily:'Ubuntu-Regular',width:'95%',alignSelf:'center',paddingLeft:10,color:'black'}}>{selleraddress.address}</Text> }
              { selleraddress=='NA' && <Text style={{fontSize:13,paddingLeft:10,fontFamily:'Ubuntu-Regular',width:'95%',alignSelf:'center',paddingLeft:10,color:'black'}}>{this.state.address}</Text> }
